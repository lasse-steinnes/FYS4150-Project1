...
for i in range(len(files[0])):
        ...
        u_num = (num,num2,num3)
        n = (n1, n2, n3)
        exac = (exact, exact2, exact3)
        row, col = np.shape(exac)
        visualize(h ,l2s, int(n1[i]), x, x_s, u_num, u_exact)
        for j in range(row):
            rel_e[j][i] = rel_error(n[j][i], u_num[j], exac[j])
            print('The relativ error of %s is %.8f.' %(files[j][i], rel_e[j][i]))
